...
static ncclResult_t taskAppend(struct ncclComm* comm, struct ncclInfo* info) {
  ...
  if (info->coll == ncclFuncSend || info->coll == ncclFuncRecv) {
    ...
    if (comm->nRanks == 1) {
      ...
      printf("[ROCM Debug]: %d %d %p %p %d %d %d\n",comm->symmetricSupport,comm->nNodes, sendWin,recvWin,(sendWin->winFlags & recvWin->winFlags & NCCL_WIN_COLL_SYMMETRIC), comm->config.CTAPolicy == NCCL_CTA_POLICY_ZERO,ceImplemented);fflush(stdout);
      ...
      if (comm->symmetricSupport && comm->nNodes == 1 && sendWin && recvWin && (sendWin->winFlags & recvWin->winFlags & NCCL_WIN_COLL_SYMMETRIC) && comm->config.CTAPolicy == NCCL_CTA_POLICY_ZERO && ceImplemented) {
        puts("[ROCM Debug] ceCollTaskAppend enter");
